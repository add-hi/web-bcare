...
export default function useTicket() {
    const {
        ...
        setListLoading, setListError, setTickets, setPagination,    
    } = useTicketStore();
    ...
    const fetchTickets = useCallback(async ({ limit = 10, offset = 0 } = {}) => {
        setListLoading(true);
        setListError(null);
        try {
            ...
            if (!payload?.success) throw new Error(payload?.message || "Gagal mengambil tiket");
            ...
            setListError(e?.response?.data?.message || e?.message || "Terjadi kesalahan saat mengambil tiket");
        } finally {
