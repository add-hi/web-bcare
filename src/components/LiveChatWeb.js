...
export default function LiveChatWeb({ room = "general" }) {
    ...
    const ACTIVE_ROOM = useMemo(() => dmRoom || fallbackCallRoom, [dmRoom, fallbackCallRoom]);
    ...
    const initialMessages = [
        {
            id: 1,
            text:
                "Halo! Saya BNI Assistant. Saya siap membantu Anda dengan keluhan atau masalah perbankan. Bisa ceritakan masalah yang Anda alami?",
            isBot: true,
            timestamp: "10:30",
        },
        { id: 2, text: "Halo, saya mengalami masalah dengan kartu ATM saya", isBot: false, timestamp: "10:31" },
        {
            id: 3,
            text:
                "Baik, saya akan membantu Anda dengan masalah kartu ATM. Bisa dijelaskan lebih detail masalah apa yang Anda alami?",
            isBot: true,
            timestamp: "10:31",
        },
        { id: 4, text: "Kartu ATM saya tertelan di mesin ATM BNI Thamrin", isBot: false, timestamp: "10:32" },
        {
            id: 5,
    ...
    const pushMsg = useCallback(
        (msg) => {
            setMessages((prev) => {
                ...
                try {
                    const pure = next.filter((m) => !initialMessages.find((im) => im.id === m.id));
    ...
    useEffect(() => {
        try {
            ...
            const merged = [...initialMessages, ...uniq.filter((m) => !initialMessages.find((im) => im.id === m.id))];
            setMessages(merged);
        } catch {
    ...
    const clearAll = () => {
        ...
        try {
            const keys = Object.keys(localStorage).filter((k) => k.startsWith("msgs:"));
        ...
        <div style={styles.container}>
            ...
            <div style={styles.header}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                    {isLiveChat && (
                        <button style={styles.ghostDanger} onClick={clearAll} title="Akhiri chat">
                            Akhiri
                        </button>
                    )}
                </div>
            ...
            <div style={styles.chatArea}>
                {messages.map((m) => (
                    <div key={m.id} style={{ display: "flex", justifyContent: m.isBot ? "flex-start" : "flex-end" }}>
                        <div style={{ ...styles.bubble, ...(m.isBot ? styles.botBubble : styles.meBubble) }}>
                            <div>{m.text}</div>
            ...
            <div style={styles.inputRow}>
                ...
                />
                <button style={isLiveChat ? styles.send : styles.sendDisabled} onClick={handleSend} disabled={!isLiveChat}>
                    Kirim
                </button>
            </div>
                ...
                <div style={styles.callOverlay}>
                    <div style={styles.callBox}>
                        <h3 style={{ margin: 0, marginBottom: 8 }}>
                            {callStatus === "ringing" ? "Panggilan Berdering..." : "Dalam Panggilan"}
                        </h3>
                        <div style={styles.videoRow}>
                            ...
                            <div style={styles.videoPane}>
                                {remoteFrame ? (
                                    ...
                                    <div style={styles.videoPlaceholder}>Menunggu frame lawan…</div>
                                )}
